...
**`src/tasks/agent.ts`** — Provider dispatch layer. `resolveAgentProvider(task)` resolves the provider in this order: (1) `task.provider` field, (2) `EXECUTANT_PROVIDER` env var, (3) `"claude"` default. `resolveAgentModel(task)` resolves the model the same way (`task.model`, then `EXECUTANT_MODEL`, else `undefined` — the provider CLI's default); it is shared by both backends and by the telemetry span attributes. `runAgent(task)` and `runAgentStructured(task, schema, onEvent?)` route to the appropriate backend and are the only entry points used by `runner.ts`, `plan.ts`, and `refine.ts`. A structured call returns a value rather than streaming, so `onEvent` is the seam its events leave by — the judge uses it to report grading cost and token usage against the step it graded, which previously went uncounted. `resolveJudgeProvider()` resolves the backend that grades `llm_as_judge` steps from `EXECUTANT_JUDGE_PROVIDER`, defaulting to `"claude"`; it deliberately ignores `EXECUTANT_PROVIDER`, so running every step on a small local model does not hand that model its own marking. Adding a new provider requires only a new case in each switch and a new `src/tasks/<provider>.ts` file.

**`src/tasks/claude.ts`** — Spawns the Claude CLI as a child process and streams its NDJSON output as `Event`s. Handles tool call parsing, cost events, token usage (`output:usage`, parsed from the result message's `usage` object — missing or malformed usage yields no event rather than throwing, since token reporting is best-effort), the account's usage-limit state (`output:rate-limit`, parsed from the CLI's `rate_limit_event` — an unknown `status` or non-object payload is dropped, not thrown, since the CLI adds statuses over time), and structured output (`output:structured`). `runClaude(task: ClaudeTask)` is the low-level generator. `runClaudeStructured<T>(task, schema)` is a typed wrapper that passes a Zod schema as `--json-schema` (via `toAgentJsonSchema`, which strips the `$schema` dialect key the CLI has no use for) and validates the result. `--json-schema` is the primary path, but a non-zero exit — most often `error_max_structured_output_retries`, where the grammar rejected the model's answer until the CLI's retry budget ran out — is caught rather than rethrown immediately, and the collected text is searched for a schema-valid object first (see `src/tasks/structured.ts`). The CLI error is rethrown only when nothing usable is there. Exports `METHODOLOGY` (the development loop loaded from `src/prompts/development-methodology.txt`) and `buildClaudeArgs(task, interactive?)` (pure function constructing the CLI args array, exported for testing). `ClaudeTask` carries runtime fields not present in YAML: `provider` (optional — routes through `agent.ts` dispatch), `permissionMode`, `jsonSchema`, `appendSystemPrompt`, `sessionId` (`--session-id`), `resume` (`--resume` — the prompt still goes in on stdin as the resumed session's next turn), `mcpConfig` (`--mcp-config`), `model`, and `agent` (OpenCode `--agent` flag). The spawn env includes `traceparentEnv()` from `src/lib/trace-context.ts`, so the subprocess inherits `TRACEPARENT` when telemetry is active.

**`src/tasks/opencode.ts`** — Spawns the OpenCode CLI (`opencode run --format json`) and streams its JSON events as `Event`s. `buildOpenCodeArgs(task)` constructs the args array (model from `task.model` then `EXECUTANT_MODEL` env; agent from `task.agent` then `EXECUTANT_AGENT` env; `--dangerously-skip-permissions` for `bypassPermissions` mode). `buildOpenCodePermissionEnv(allowedTools)` translates the `allowed_tools` step field into the `OPENCODE_PERMISSION` env var: `undefined` → no env set (all tools allowed); `[]` → deny all tools (text-only mode); `["bash","read"]` → deny every tool not in the list. Tool names are matched case-insensitively so Claude-style names (`Bash`, `Read`) and opencode-style names (`bash`, `read`) both work. `parseOpenCodeMessage(msg)` normalises OpenCode's event types (`text`, `tool_use`, `error`) to Executant's `output:text` and `output:tool` events. `runOpenCodeStructured` appends a JSON-only instruction to the prompt and recovers the object through the same shared `salvageStructured` the Claude path uses, so a provider swap does not change how forgiving a structured step is. The spawn env includes `traceparentEnv()` alongside `OPENCODE_PERMISSION`.

**`src/tasks/structured.ts`** — Provider-neutral structured-output recovery, shared by every backend so each one does not invent its own idea of how forgiving to be. `findJsonObjects(text)` returns every balanced top-level `{…}` span in order (brace-matching, not a regex, and string/escape aware — the output of a failed structured call holds several rejected attempts back to back, and the span from the first `{` to the last `}` is the one thing guaranteed not to parse). `salvageStructured(text, schema)` returns the last candidate that validates, or `undefined`. Newest-first because each attempt was written after seeing why the previous one was rejected; a candidate that fails validation gets one retry unwrapped, since agents intermittently nest the whole answer under a single key (`{"output": {…}}`) instead of returning it at the root.
...
**`src/lib/statusline.ts`** — The context gauge's pure core plus one git read. `statusLineEnabled(env?)` reads `EXECUTANT_STATUSLINE` (default on). `contextWindowSize(model)` returns 200,000, or 1,000,000 for a model whose id carries the `[1m]` suffix. `contextTokens(usage)` sums input + cache creation + cache read — the tokens that occupy a context window; output tokens do not. It is only ever valid for one call's usage, which is why `src/tasks/claude.ts` applies it to the per-call `assistant` messages rather than the cumulative `result` totals. `buildGauge(tokens, size, width?)` produces the whole rendered line's data: `pct` (floored, capped at 100 — truncating rather than rounding so the bar never overstates and never crosses a colour threshold early), the `filled`/`empty` halves of the 10-cell bar, a `level` of `ok`/`warn`/`high` at the 70%/90% boundaries, and the `used`/`limit` labels (`162.2k`, `200k`, `1M`). `readRepoInfo(cwd)` gets repo name and branch from a single `git rev-parse --show-toplevel --abbrev-ref HEAD`, substituting the short SHA for a detached HEAD's `"HEAD"`, and resolving `undefined` outside a repository so the bar just drops that segment.
...
**`src/tests/ink-harness.ts`** — shared helpers for every test that renders the real TUI (not a `*.test.ts` file, so the suite glob ignores it). A mounted Ink tree holds timers and stdin listeners, so an assertion that throws *before* `unmount()` leaves the process unable to exit — and under `node --test` one such file wedges the entire run indefinitely instead of reporting a failure. `withInk(element, body)` makes the unmount unconditional; `renderTracked` + `afterEach(unmountAllInk)` do the same for files whose tests share a render factory. `waitForFrame(frame, match, opts?)` polls for a condition and **throws** with the last frame on timeout, instead of returning a stale frame for an assertion to fail on confusingly; its deadline is deliberately generous (15s) because the suite runs 40+ node processes in parallel, each with its own esbuild transform service, and a frame assertion racing a fixed sleep is exactly what made this flaky. `settle()` remains for the assertions that check something must *not* appear, which have no condition to poll for. `--test-timeout=60000` in the `test` script is the backstop for anything that still manages to hang.
...
- `output:usage` — token usage (`inputTokens`/`outputTokens`/`cacheCreationTokens`/`cacheReadTokens`) parsed from the Claude CLI's final `result` message, alongside `output:cost`; absent when the CLI doesn't report it. **Cumulative across every API call the step made**, so it measures throughput and feeds the run report's totals — never a context gauge.
- `output:context` — how full the running session's context window is (`input + cacheCreation + cacheRead`), re-emitted as the session grows. Read from the stream's `assistant` messages and deduplicated by message id (the CLI emits one per content block, all sharing that turn's usage). Consumers **replace** rather than accumulate: a session's turns are one conversation re-measured, not amounts to sum. This drives the TUI's context gauge.
- `output:rate-limit` — the provider's own view of the account's usage limit, forwarded from the Claude CLI's `rate_limit_event` whenever it changes: `status` (`allowed` / `allowed_warning` / `rejected`), plus optional `resetsAt` (unix seconds), `rateLimitType`, `utilization` (0..1) and `windows.five_hour` / `windows.seven_day` (`{ utilization, resetsAt }`). Carries the step `index`, patched like the other `output:*` sentinels. Consumers **replace** rather than accumulate — each event is the latest state, not an increment. Not billable (never feeds `output:cost`/`output:usage` aggregation). Executant only relays it — it never pauses or switches on it — so an orchestrator can act before a run hard-stops with "usage limit reached".
- `output:structured` — schema-validated JSON object from a Claude invocation that used `--json-schema`. Absent when the CLI could not produce one, which is not fatal on its own: `runClaudeStructured` falls back to recovering the object from the invocation's text output.
