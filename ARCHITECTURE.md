...
**`src/lib/statusline.ts`** — Best-effort integration with Claude Code's `statusLine` setting. `findStatusLineCommand(cwd, home?)` walks up from `cwd` reading `.claude/settings.local.json` then `.claude/settings.json` at each level (falling back to the same pair under `home`), returning the first `statusLine.command` found. `buildStatusLinePayload(opts)` synthesizes the same JSON shape Claude Code sends its own statusline commands (session id, model, cwd, an approximate running cost) since executant is not an interactive session and has no real equivalent. `runStatusLine(command, payload, timeoutMs?)` spawns the command through the shell with the payload on stdin and returns its first stdout line, or `undefined` on any error, timeout, non-zero exit, or empty output. `statusLineEnabled(env?)` reads `EXECUTANT_STATUSLINE` (default on). Every function is designed to never throw — a broken statusline should never break a run.

**`src/ui/useStatusLine.ts`** — Hook wrapping `statusline.ts` for `App.tsx`: resolves the command and a session id once via `useState` initializers, then runs it on mount and every 30s via `setInterval`, storing the latest non-empty output line in state (stale output is kept, not cleared, between refreshes). Reads `totalCostUsd`/`startTime` through a ref inside the interval closure so cost/tick updates don't re-trigger the effect. Returns `null` when disabled, unconfigured, or no output has landed yet, which `App.tsx` treats as "render nothing."
...
- `output:usage` — token usage (`inputTokens`/`outputTokens`/`cacheCreationTokens`/`cacheReadTokens`) reported alongside `output:cost`. Parsed from the Claude CLI's `usage` object; absent when the CLI doesn't report it.
