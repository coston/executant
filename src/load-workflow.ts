...
function convertInnerStep(step: RawStep, vars: Record<string, string>, name: string, continueOnError: boolean): CommandTask | ClaudeTask | LogTask {
...
function resolveVarPath(varName: string, vars: Record<string, string>, stepName: string, label: string): string {
  if (!(varName in vars)) {
...
function resolveOutputFile(varName: string, vars: Record<string, string>, stepName: string): string {
