...
function tmpYaml(content: string): string {
  ...
  const file = join(dir, `${Date.now()}-${Math.random().toString(36).slice(2, 8)}.yaml`);
...
describe('reducer — output:tool formatting', () => {
  ...
  test('Read tool shows file path', () => {
    const line = formatViaReducer(runningState(), 'Read', { file_path: '/src/index.ts' });
    assert.equal(line, '[Read] /src/index.ts');
  });

  test('Edit tool shows file path', () => {
    const line = formatViaReducer(runningState(), 'Edit', { file_path: '/src/app.ts' });
    assert.equal(line, '[Edit] /src/app.ts');
  });

  test('Write tool shows file path', () => {
    const line = formatViaReducer(runningState(), 'Write', { file_path: '/tmp/out.txt' });
    assert.equal(line, '[Write] /tmp/out.txt');
  });

  test('Bash tool shows description and command', () => {
    const line = formatViaReducer(runningState(), 'Bash', {
      description: 'Run tests',
      command: 'npm test',
    });
    assert.ok(line.includes('[Bash] Run tests'), `expected description, got: ${line}`);
    assert.ok(line.includes('$ npm test'), `expected command, got: ${line}`);
  });

  test('Glob tool shows pattern', () => {
    const line = formatViaReducer(runningState(), 'Glob', { pattern: '**/*.ts' });
  ...
  test('Agent tool does not leak prompt JSON', () => {
    ...
    assert.ok(!line.includes('Find all the things'), `prompt should not appear in output: ${line}`);
  ...
  test('unknown tools show JSON in debug mode', () => {
    ...
    const line = formatViaReducer(runningState(), 'SomeNewTool', { foo: 'bar' });
    assert.ok(line.includes('[SomeNewTool]'), `expected tool name, got: ${line}`);
  ...
  test('TodoWrite shows in-progress tasks', () => {
    ...
    assert.ok(line.includes('working on this'), `expected task content, got: ${line}`);
  });
...
describe('reducer — core state transitions', () => {
  function buildState(): ExecutionState {
    return buildInitialState(loadWorkflow(tmpYaml(`
goal: test
steps:
  - name: step-a
    command: echo a
...
`)));
  ...
  test('buildInitialState sets all tasks to pending', () => {
    ...
    assert.ok(state.tasks.every((t) => t.status === 'pending'), 'all tasks should be pending');
  });
  ...
  test('workflow:start updates startTime', () => {
    ...
    const next = reducer(state, { type: 'workflow:start', workflow: state.workflow });
  ...
  test('workflow:complete sets endTime', () => {
    const state = buildState();
    const before = Date.now();
    const next = reducer(state, { type: 'workflow:complete', workflow: state.workflow, durationMs: 100 });
    assert.ok((next.endTime ?? 0) >= before, 'endTime should be set');
  });

  test('step:start marks the task as running and sets startTime', () => {
    const state = buildState();
    const next = reducer(state, { type: 'step:start', index: 0, name: 'step-a' });
  ...
  test('step:complete marks task complete and advances currentIndex', () => {
    ...
    state = reducer(state, { type: 'step:complete', index: 0, name: 'step-a', durationMs: 10 });
  ...
  test('step:error marks task errored and advances currentIndex', () => {
    let state = buildState();
    state = reducer(state, { type: 'step:start', index: 0, name: 'step-a' });
    state = reducer(state, { type: 'step:error', index: 0, name: 'step-a', error: new Error('boom') });
  ...
  test('output:text with index beyond tasks.length is a no-op', () => {
    const state = buildState();
    const next = reducer(state, { type: 'output:text', index: 999, text: 'oob' });
    assert.deepEqual(next, state);
  });
  ...
  test('log event appends line to currentIndex task', () => {
    ...
    assert.ok(state.tasks[0].lines.some((l) => l.includes('[info] starting up')));
  });

  test('log event uses [warn] prefix for warn level', () => {
    let state = buildState();
    state = reducer(state, { type: 'step:start', index: 0, name: 'step-a' });
    state = reducer(state, { type: 'log', level: 'warn', text: 'something degraded' });
    assert.ok(state.tasks[0].lines.some((l) => l.startsWith('[warn]')));
  });

  test('lines accumulate without truncation', () => {
    let state = buildState();
    state = reducer(state, { type: 'step:start', index: 0, name: 'step-a' });
    for (let i = 0; i < 250; i++) {
      state = reducer(state, { type: 'output:text', index: 0, text: `line ${i}` });
    }
