...
`,
  ...
  test("resolves a context var whose own value nests another var placeholder", () => {
    const dir = tmpDir();
    // docs_dir isn't a literal path here — notes' raw value nests {{docs_dir}},
    // the same pattern real workflows use (e.g. ticket_file: "{{docs_dir}}/ticket.md").
    const yaml = tmpYaml(
      dir,
      `
goal: test
...
steps:
  - name: analyze
    context: [notes]
    prompt: Analyze the notes above.
`,
    );
    ...
    assert.deepEqual(task.contextFiles, [join(dir, "notes.txt")]);
  });
});
...
describe("context injection — prompt expansion", () => {
  test("file contents are prepended inside a labelled fence", async () => {
    ...
    assert.ok(
      expanded.prompt.indexOf("KNOWN_CONTENT") <
        expanded.prompt.indexOf("ORIGINAL_PROMPT"),
  ...
  test("multiple context files are each wrapped in labelled fences", async () => {
    ...
    assert.ok(
      expanded.prompt.includes(`### ${fileA}`),
      "must include header for file A",
    );
    assert.ok(
      expanded.prompt.includes("CONTENT_A"),
      "must include contents of file A",
    );
    assert.ok(
      expanded.prompt.includes(`### ${fileB}`),
      "must include header for file B",
    );
    assert.ok(
      expanded.prompt.includes("CONTENT_B"),
      "must include contents of file B",
    );
    assert.ok(
      expanded.prompt.includes("BASE_PROMPT"),
      "must retain original prompt",
    );
    // Both files must appear before the base prompt
    assert.ok(
      expanded.prompt.indexOf("CONTENT_B") <
        expanded.prompt.indexOf("BASE_PROMPT"),
    );
  });
