...
`);
      ...
      const inner = (wf.tasks[0] as ForEachTask).inner[0] as CommandTask;
...
`);
      ...
      assert.equal(
        (wf.tasks[0] as unknown as Record<string, unknown>)["selfHealing"],
        undefined,
      );
    });
  });
  ...
  describe("runWorkflow — self-healing retry loop", () => {
    ...
    test("invokes Claude on failure and retries", async () => {
      ...
      assert.ok(
        logs.some(
          (e) =>
            e.text.includes("[self-healing]") && e.text.includes("Attempt 1/2"),
        ),
    ...
    test("succeeds when command passes on second attempt", async () => {
      ...
      assert.ok(
        logs.some(
          (e) =>
            e.text.includes("[self-healing]") && e.text.includes("Attempt 1/3"),
        ),
    ...
    test("continue_on_error lets workflow continue after exhaustion", async () => {
      ...
      assert.ok(
        events.some(
          (e) =>
            e.type === "step:error" && "name" in e && e.name === "will_exhaust",
        ),
...
`,
    ...
    test("records tool calls as fix summary in subsequent attempt prompt", async () => {
      ...
      assert.ok(
        prompts.length >= 2,
        `Expected at least 2 Claude invocations, got ${prompts.length}`,
      );
...
`,
  ...
  describe("self-healing prompt template", () => {
    test("self-healing-fix.txt exists and has required placeholders", () => {
      ...
      assert.ok(
        content.includes("{{ATTEMPT_HISTORY}}"),
        "Missing {{ATTEMPT_HISTORY}} placeholder",
      );
    });
...
`),
      ...
      assert.equal(
        task.selfHealing,
        false,
        "loader must set selfHealing=false by default",
      );
...
`),
      ...
      assert.equal(
        task.selfHealing,
        true,
        "loader must set selfHealing=true when specified",
      );
...
`),
      ...
      assert.equal(
        ...
        false,
        "continueOnError must default to false when omitted",
      );
