...
describe("findProjectRoot", () => {
  ...
  beforeEach(() => {
    tmpRoot = join(
      tmpdir(),
      `executant-plan-test-${process.pid}-${Date.now()}`,
    );
...
describe("parsePlanArgs — argument errors", () => {
  ...
  test("exits 1 when -f file does not exist", () => {
    assert.throws(
      () => parsePlanArgs(["-f", "/nonexistent/path/file.txt"]),
      /process\.exit\(1\)/,
    );
    assert.equal(exitCode, 1);
  ...
  test("exits 1 when --file file does not exist", () => {
    assert.throws(
      () => parsePlanArgs(["--file", "/no/such/file.yaml"]),
      /process\.exit\(1\)/,
    );
    assert.equal(exitCode, 1);
  });
...
describe("isSimpleRequest", () => {
  test('detects "N times" pattern', () => {
    assert.ok(
      isSimpleRequest("repeat the following prompt 20 times: check the file"),
    );
  });
...
describe("normalizeWorkflow", () => {
  ...
  test('converts forEach "seq 1 N" shell command to repeat', () => {
    const workflow = {
      ...base,
      steps: [
        { name: "step", prompt: "do thing pass {{item}}", forEach: "seq 1 20" },
      ],
    };
    const result = normalizeWorkflow(workflow);
  ...
  test('converts labeled sequential forEach ["pass 1","pass 2","pass 3"] to repeat', () => {
    const workflow = {
      ...
      steps: [
        {
          name: "step",
          prompt: "do thing {{item}}",
          forEach: ["pass 1", "pass 2", "pass 3", "pass 4"] as string[],
        },
      ],
    };
    const result = normalizeWorkflow(workflow);
      ...
      steps: [
        {
          name: "step",
          prompt: "do thing",
          forEach: ["iteration 1", "iteration 2", "iteration 3"] as string[],
        },
      ],
    };
    const result = normalizeWorkflow(workflow);
      ...
      steps: [
        {
          name: "step",
          prompt: "do thing",
          forEach: ["pass 1", "pass 3"] as string[],
        },
      ],
    };
    const result = normalizeWorkflow(workflow);
    assert.deepEqual((result.steps[0] as { forEach?: unknown }).forEach, [
      "pass 1",
      "pass 3",
    ]);
  });
  ...
  test('does not convert forEach "seq" with non-1 start', () => {
    ...
    const result = normalizeWorkflow(workflow);
    assert.equal(
      (result.steps[0] as { forEach?: unknown }).forEach,
      "seq 5 20",
    );
  });
  ...
  test("does not collapse sequential steps that already have forEach/repeat", () => {
    const workflow = {
      ...base,
      steps: [{ name: "step_1", prompt: "do thing", repeat: 2 }],
    };
    const result = normalizeWorkflow(workflow);
  ...
  test("infers repeat from step name when prompt uses {{item}} but no forEach/repeat", () => {
    const workflow = {
      ...base,
      steps: [
        { name: "review_pass_of_5", prompt: "Review pass {{item}} of 5." },
      ],
    };
    const result = normalizeWorkflow(workflow);
    assert.equal((result.steps[0] as { repeat?: number }).repeat, 5);
      ...
      steps: [
        {
          name: "step",
          prompt: "do thing",
          forEach: ["foo.ts", "bar.ts"] as string[],
        },
      ],
    };
    const result = normalizeWorkflow(workflow);
  ...
  test("preserves nested steps: array when forEach is numeric (converts to repeat, keeps steps)", () => {
    const workflow = {
      ...base,
      steps: [
        {
          name: "verify",
          forEach: ["1", "2", "3"] as string[],
          steps: [
            {
              name: "lint {{item}}",
    ...
    const result = normalizeWorkflow(workflow);
    const step = result.steps[0] as { repeat?: number; steps?: unknown[] };
    assert.equal(step.repeat, 3);
    assert.ok(Array.isArray(step.steps));
    assert.equal(step.steps?.length, 2);
  });

  test("collapseSequentialSteps does not collapse forEach+steps steps even if named with _1 suffix", () => {
    const workflow = {
      ...base,
      steps: [
        {
    ...
    const result = normalizeWorkflow(workflow);
    // Should remain as two separate steps, not collapsed into repeat: 2
    assert.equal(result.steps.length, 2);
    assert.equal(result.steps[0]!.name, "process_1");
    assert.equal(result.steps[1]!.name, "process_2");
  });
});
...
describe("parsePlanArgs — auto-creates tasks directory", () => {
  ...
  beforeEach(() => {
    ...
    tmpRoot = join(
      tmpdir(),
      `executant-autocreate-${process.pid}-${Date.now()}`,
    );
    mkdirSync(tmpRoot, { recursive: true });
    originalExit = process.exit;
    (process as NodeJS.Process).exit = ((code?: number) => {
...
function installPlanMock(responses: MockResponse[]): {
  counterFile: string;
  originalPath: string;
} {
...
const VALID_WORKFLOW_NO_TESTS = {
  ...
  steps: [
    {
      name: "implement",
      prompt: "Implement user authentication with JWT — no verification.",
    },
  ],
};

const JUDGE_PASS = JSON.stringify({ pass: true, feedback: "" });
const JUDGE_FAIL_NO_TESTS = JSON.stringify({
  pass: false,
  feedback: "Missing lint, test, and build verification steps.",
});
...
describe("streamPlan", () => {
  ...
  function makePlanArgs(
    slug = "test-task",
    opts: Partial<PlanArgs> = {},
  ): PlanArgs {
    return {
  ...
  test("success: writes YAML with expected goal and steps (3 invocations: research, decompose, judge)", async () => {
    ...
    assert.ok(
      ...
      "Expected no plan:error event",
    );
    assert.ok(
      !events.some((e) => e.type === "plan:retry"),
      "Expected no plan:retry event",
    );
  });
  ...
  test("judge rejects on first attempt, Pass 2 retried with feedback", async () => {
    ...
    assert.equal(
      readFileSync(counterFile, "utf8").trim(),
      "5",
      "Expected exactly 5 invocations",
    );

    assert.ok(
      existsSync(args.taskFile),
    ...
    assert.equal(
      stage2Events.length,
      2,
      "Expected stage 2 to appear twice (initial + retry)",
    );
  });
  ...
  test("Pass 1 failure yields plan:error immediately — no decompose or judge invocation", async () => {
    ...
    assert.equal(
      readFileSync(counterFile, "utf8").trim(),
      "1",
      "Expected exactly 1 Claude invocation",
    );
    ...
    assert.ok(
      !existsSync(args.taskFile),
      "Expected no YAML file after Pass 1 failure",
    );
    assert.ok(
      !events.some((e) => e.type === "plan:complete"),
      "Expected no plan:complete event",
    );
  ...
  test("Pass 2 retry: invalid decompose response on attempt 1, success on attempt 2", async () => {
    ...
    assert.equal(
      readFileSync(counterFile, "utf8").trim(),
      "4",
      "Expected exactly 4 Claude invocations",
    );

    assert.ok(
      existsSync(args.taskFile),
      "Expected YAML file to be written after decompose retry",
    );

    const retryEvent = events.find((e) => e.type === "plan:retry") as
      | Extract<PlanEvent, { type: "plan:retry" }>
      | undefined;
  ...
  test("judge rejects on all attempts: YAML still written on final attempt (non-blocking)", async () => {
    ...
    assert.equal(
      readFileSync(counterFile, "utf8").trim(),
      "7",
      "Expected exactly 7 Claude invocations",
    );
    ...
    assert.equal(
      retryEvents.length,
      2,
      "Expected 2 plan:retry events (attempts 2 and 3)",
    );
  ...
  test("fast path: only 2 invocations (decompose + judge), no research pass", async () => {
    ...
    const args = makePlanArgs("fast-repeat", {
      fast: true,
      description: "repeat the task 10 times",
    });
    const events = await collectPlanEvents(args);
    ...
    assert.equal(
      readFileSync(counterFile, "utf8").trim(),
      "2",
      "Expected exactly 2 Claude invocations for fast path",
    );
    ...
    assert.equal(
      stages.length,
      2,
      "Expected exactly 2 stage events in fast path",
    );
    assert.equal(stages[0]!.name, "Decompose to Steps");
    assert.equal(
      stages[0]!.stage,
      1,
      "Decompose should be stage 1 in fast path",
    );
    ...
    assert.equal(
      stages[1]!.stage,
      2,
      "Validate should be stage 2 in fast path",
    );
  ...
  test("auto-detects simple request and uses fast path", async () => {
    ...
    assert.equal(
      readFileSync(counterFile, "utf8").trim(),
      "2",
      "Expected 2 invocations: auto-detected simple request skips research",
    );
    assert.ok(
      events.some((e) => e.type === "plan:complete"),
      "Expected plan:complete event",
  ...
  test("three consecutive decompose failures: yields plan:error and writes no file", async () => {
    ...
    assert.ok(
      !existsSync(args.taskFile),
      "Expected no YAML file after 3 decompose failures",
    );
    assert.ok(
      !events.some((e) => e.type === "plan:complete"),
      "Expected no plan:complete event",
