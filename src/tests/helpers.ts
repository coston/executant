...
export function tmpDir(): string {
  const dir = join(tmpdir(), `executant-test-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
...
export async function collectEventsUntilError(workflow: Workflow): Promise<{ events: Event[]; error?: Error }> {
  const { runWorkflow } = await import('../runner.js');
  const events: Event[] = [];
  try {
    for await (const e of runWorkflow(workflow)) events.push(e);
...
export function installMockClaude(): { mockDir: string; originalPath: string } {
  const mockDir = join(tmpdir(), `executant-mock-claude-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
